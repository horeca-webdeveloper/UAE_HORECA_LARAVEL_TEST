...
class RenderingSiteMapListener
{
    public function handle(RenderingSiteMapEvent $event): void
    {
        if ($key = $event->key) {
                ...
                case 'product-tags':
                    ...
                    case 'product-types':
                        ...
                        foreach ($types as $types) {
                            ...
                            SiteMapManager::add($types->url, $types->updated_at, '0.3', 'weekly');
                        }
            ...
            $productTypesUpdated = ProductTypes::query()
            ->selectRaw('MAX(updated_at) as updated_at')
            ->wherePublished()
            ->latest('updated_at')
            ->value('updated_at');

        if ($productTypesUpdated) {
            SiteMapManager::addSitemap(SiteMapManager::route('product-types'), $productTypesUpdated);
        }
        }
    }
}
