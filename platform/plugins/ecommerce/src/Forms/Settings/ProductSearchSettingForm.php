...
class ProductSearchSettingForm extends SettingForm
{
    public function setup(): void
    {
        ...
        $this
            ...
            ->add(
                'enable_filter_products_by_producttypes',
                OnOffCheckboxField::class,
                OnOffFieldOption::make()
                    ->label(trans('plugins/ecommerce::setting.product_search.form.enable_filter_products_by_producttypes'))
                    ->value(EcommerceHelper::isEnabledFilterProductsByProductTypes())
                    ->defaultValue(true)
                    ->toArray()
            )
            ->add(
