    ...
    public function update(
        ...
        foreach ($testimonials as $testimonial) {
            $review = Review::where('product_id', $product->id)
                            ->where('customer_name', $testimonial['customer_name'])
                            ->first();
            ...
            $review->customer_name = $testimonial['customer_name'];
            $review->star = $isAdmin ? $testimonial['star'] : (empty($testimonial['star']) ? null : $testimonial['star']);
            $review->comment = $testimonial['comment'];
            $review->status = $testimonial['status'] ?? 'active'; // Use provided status or default to 'active'
            $review->images = $testimonial['images'] ?? [];
            $review->save();
        }
    }
