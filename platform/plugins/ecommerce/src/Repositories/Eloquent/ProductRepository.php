...
class ProductRepository extends RepositoriesAbstract implements ProductInterface
{
    ...
    public function getProductByType(array $params)
    {
        $params = array_merge([
            'product_type' => [
                'by' => 'id',
        ...
        $filters = ['producttypes' => $params['product_type']['value_in']];

        Arr::forget($params, 'product_type');
    ...
    public function filterProducts(array $filters, array $params = [])
    {
            ...
            if (is_plugin_active('language') && is_plugin_active('language-advanced') && Language::getCurrentLocale() != Language::getDefaultLocale()) {
                $this->model = $this->model
                    ->where(function (EloquentBuilder $query) use ($keyword, $searchProductsBy, $isPartial) {
                        ...
                        if (in_array('producttypes', $searchProductsBy)) {
                            $function = $hasWhere ? 'orWhereHas' : 'whereHas';
                            $hasWhere = true;

                            $query->{$function}('producttypes', function (EloquentBuilder $query) use ($keyword) {
                                $query->where(function (BaseQueryBuilder $subQuery) use ($keyword) {
                                    $subQuery->addSearch('name', $keyword, false);
                                });
                            });
        ...
        if ($filters['tags']) {
         ...
         // Filter product by tags
         $filters['producttypes'] = array_filter($filters['producttypes']);
         if ($filters['producttypes']) {
             $this->model = $this->model
                 ->whereHas('producttypes', function (EloquentBuilder $query) use ($filters) {
                     return $query
                         ->whereIn('ec_products_product_types_product.producttypes_id', $filters['producttypes']);
                 });
         }
