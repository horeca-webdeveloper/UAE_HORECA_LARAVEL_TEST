    ...
    public function handle(Slug|array $slug): array|Slug
    {
        ...
        switch ($slug->reference_type) {
            case Product::class:
                ...
                if (function_exists('admin_bar')) {
                    admin_bar()
                        ->registerLink(
                            trans('plugins/ecommerce::products.edit_this_product'),
                            route('products.edit-content', $product->getKey()),
                            null,
            ...
            case ProductTypes::class:
                $condition = [
                    'ec_products_product_types.id' => $slug->reference_id,
                    'ec_products_product_types.status' => BaseStatusEnum::PUBLISHED,
                ];

                if ($isPreview) {
                    Arr::forget($condition, 'ec_products_product_types.status');
                }
                ...
                $request->merge([
                    'producttypes' => [$tag->getKey()],
                ]);

                $request->merge([
                    'producttypes' => [$producttyp->getKey()],
                ]);
                ...
                SeoHelper::setTitle($producttyp->name)->setDescription($producttyp->description);
                ...
                if (function_exists('admin_bar')) {
                    admin_bar()
                        ->registerLink(
                            trans('plugins/ecommerce::product-tag.edit_this_product_types'),
                            route('product-types.edit', $tag->getKey()),
                            null,
                            'product-types.edit'
                        );
                }
                ...
                case ProductTag::class:
                    ...
                    case ProductTypes::class:
                        ...
                        $meta = new SeoOpenGraph();
                        $meta->setDescription($tagproducttyp->description);
                        $meta->setUrl($producttyp->url);
                        $meta->setTitle($producttyp->name);
        
                        SeoHelper::setSeoOpenGraph($meta);
        
                        SeoHelper::meta()->setUrl($producttyp->url);
                        ...
                        if (function_exists('admin_bar')) {
                            admin_bar()
                                ->registerLink(
                                    trans('plugins/ecommerce::product-tag.edit_this_product-types'),
                                    route('product-types.edit', $producttyp->getKey()),
                                    null,
                                    'product-types.edit'
                                );
                        }
                        ...
                        return [
                            'view' => 'ecommerce.product-types',
                            'default_view' => 'plugins/ecommerce::themes.product-types',
                            'data' => compact('producttypes', 'products'),
                            'slug' => $tag->slug,
                        ];
